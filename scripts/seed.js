// Cross-platform seed script — works on Windows (PowerShell/CMD) and Linux/Mac.
// Run with: node scripts/seed.js  (or: bun run scripts/seed.ts)
//
// Seeds the local SQLite database with sample data for sandbox preview.
...
async function main() {
  ...
  // inactive user (Tipo = 'Users' but Status = 'i') to test login refusal
  ...
  const mes_ref = (d) => `${String(d.getMonth() + 1).padStart(2, '0')}-${d.getFullYear()}`
  ...
  // Created for yesterday, day before yesterday, and 3 days ago.
  for (let i = 1; i <= 3; i++) {
